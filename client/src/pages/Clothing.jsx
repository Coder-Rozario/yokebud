...
const useProductsData = (processProductData) => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      setIsFetching(true);
      try {
        const cachedProducts = localStorage.getItem('cachedClothingProducts');
        const cacheTimestamp = localStorage.getItem('cachedClothingTimestamp');
        ...
        localStorage.setItem('cachedClothingProducts', JSON.stringify(processedData));
        localStorage.setItem('cachedClothingTimestamp', Date.now().toString());
...
const useCategoriesData = () => {
  const [availableCategories, setAvailableCategories] = useState(['All', 'Men', 'Women', 'Kids']);
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      ...
      try {
        ...
        // Get Clothing subcategories
        const clothingSubcategories = [];
        if (data.Men) clothingSubcategories.push(...data.Men);
        if (data.Women) clothingSubcategories.push(...data.Women);
        if (data.Kids) clothingSubcategories.push(...data.Kids);
        
        setAvailableSubcategories([...new Set(clothingSubcategories)]);
      } catch (err) {
        if (err.name !== 'AbortError') {
  ...
  return { availableCategories, availableSubcategories, categoriesLoading };
};
...
const useFilters = () => {
  ...
  const initialSubcategories = searchParams.get('subcategories') ? searchParams.get('subcategories').split(',') : [];
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      if (priceRange[1] !== 200) params.set('maxPrice', priceRange[1]);
  ...
  }, [category, subcategories, priceRange, searchQuery, setSearchParams]);
...
// Optimized product filtering function
const filterProducts = (products, filters) => {
  const { category, subcategories, priceRange, searchQuery } = filters;
  ...
  return products.filter(product => {
    ...
    // Price filter - use min_price for filtering or fallback to price
    const productMinPrice = product.min_price || product.price;
    if (productMinPrice < priceRange[0] || productMinPrice > priceRange[1]) {
      return false;
    }
    ...
    if (searchQuery) {
      ...
      if (product.tags && Array.isArray(product.tags)) {
        tagMatch = product.tags.some(tag => tag.toLowerCase().includes(query));
      }
...
// Memoized product card component with lazy loading and simplified animations
const ProductCard = React.memo(({ product, onClick, isMobile, isSmallScreen, nameWidths, addToCartWithNotification, isFiltering }) => {
  ...
  // Simplified animations when filtering
  const animationProps = isFiltering ? {
              ...
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <span style={{
                    ...
                    ${product.discounted_price.toFixed(2)} - ${(product.max_price || product.discounted_price).toFixed(2)}
                  </span>
                </div>
              </>
            ) : (
...
// Filtering animation component
const FilteringAnimation = () => (
  <motion.div 
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
...
const Clothing = () => {
  ...
  // Use useTransition for filter changes to keep UI responsive
  const [isFiltering, startTransition] = useTransition();
  const [showFilteringAnimation, setShowFilteringAnimation] = useState(false);
  ...
  const { availableCategories, availableSubcategories, categoriesLoading } = useCategoriesData();
  ...
  // Optimized filtering with useTransition
  const [filteredProducts, setFilteredProducts] = useState([]);
  
  useEffect(() => {
    if (products.length === 0) return;
    
    // Show filtering animation for a brief moment
    setShowFilteringAnimation(true);
    ...
    const timeoutId = setTimeout(() => {
      startTransition(() => {
        ...
        setShowFilteringAnimation(false);
      });
    }, 100); // Small delay to show animation
    
    return () => clearTimeout(timeoutId);
  }, [products, category, subcategories, priceRange, searchQuery, startTransition]);
  ...
  // FIXED: Measure product name widths for filtered products - debounced and properly timed
  useEffect(() => {
    if (!isFetching && !error && filteredProducts.length > 0) {
      const timeoutId = setTimeout(() => {
        const newNameWidths = {};
      ...
      }, 500); // Increased debounce time to ensure DOM is ready
      
      return () => clearTimeout(timeoutId);
    }
  }, [isFetching, error, filteredProducts, isMobile, isSmallScreen]);
...
const sidebarBaseStyle = {
  ...
  scrollbarColor: 'rgb(212 175 55 / 28%) rgba(255, 255, 255, 0.05)', // Firefox (soft gold on dark)
  ...
  '::-webkit-scrollbar-thumb': {
    background: 'rgba(212, 175, 55, 0.7)', // soft gold
    borderRadius: '10px',
    border: '2px solid rgba(26, 26, 26, 0.95)', // glossy effect
  },
  '::-webkit-scrollbar-thumb:hover': {
    background: 'rgba(212, 175, 55, 0.9)', // brighter on hover
  },

  // 🚫 Hide arrows (top/bottom buttons) and corner line
      ...
      <div style={{
        ...
        >
          ...
          <motion.div 
            ...
            style={{ marginTop: '0rem', paddingTop: '0rem', borderTop: '1px solid rgba(255, 165, 0, 0.2)' }}
          >
            <motion.button 
              onClick={resetFilter}
              style={{
          ...
          >
            ...
            {availableCategories.map((cat) => (
              <motion.div
                ...
                transition={{ type: 'spring', stiffness: 300 }}
              >
                <span style={filterLabelStyle}>{cat === 'All' ? 'All Clothing' : `${cat}'s Collection`}</span>
              </motion.div>
            ))}
          </motion.div>

          {/* Subcategories Section - Simplified without scrolling */}
          <motion.div
            variants={containerVariants}
            initial="hidden"
            animate="visible"
          ...
          >
            <motion.h6 
              ...
              variants={itemVariants}
            >
              <FaTshirt style={{ marginRight: '0.75rem' }} />
              Subcategories
            </motion.h6>
            ...
            {categoriesLoading ? (
              ...
              availableSubcategories.map((sub) => (
                <motion.div
                  key={sub}
                  style={filterItemStyle(subcategories.includes(sub))}
                  onClick={() => {
                ...
                >
                  <motion.div 
                    style={checkboxStyle(subcategories.includes(sub))}
                    animate={{
                      scale: subcategories.includes(sub) ? [1, 1.1, 1] : 1
                    }}
                    transition={{ duration: 0.3 }}
                  >
                  ...
                  <span style={filterLabelStyle}>{sub}</span>
                </motion.div>
              ))
            )}
          </motion.div>
          ...
          <div style={{
            ...
            <div style={{display: 'flex', flexDirection: 'row', justifyContent:'space-between'}}>
              ...
              >
                <motion.h1 
                  style={{ margin: 0 }}
                  key={category + subcategories.join()}
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.3 }}
                >
                  {category === 'All' ? 'All Clothing Products' : (
                    <>
