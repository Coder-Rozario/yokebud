...
const AdminEdit = () => {
  ...
  const API_BASE_URL = 'https://api.yokebud.com/api';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const actualProduct = productData.product || productData;
        
        const mappedProduct = {
          name: actualProduct.product_name || actualProduct.name || '',
          description: actualProduct.product_details || actualProduct.description || '',
          price: actualProduct.price ? actualProduct.price.toString() : '', 
          min_price: actualProduct.min_price ? actualProduct.min_price.toString() : 
                    (actualProduct.discounted_price ? actualProduct.discounted_price.toString() : 
                    (actualProduct.price ? actualProduct.price.toString() : '')), 
          categories: actualProduct.categories || (actualProduct.category ? [actualProduct.category] : []),
          subcategory: actualProduct.subcategory || '',
          moq: actualProduct.moq || 1,
          material: actualProduct.material || '',
          care: actualProduct.care_instructions || actualProduct.care || '',
          sku: actualProduct.sku || '',
          shipping: actualProduct.shipping_info || actualProduct.shipping || 'Free shipping',
          warranty: actualProduct.warranty || '1 year quality guarantee',
          bulk_discount: actualProduct.bulk_discount || '10% off for 50+ units',
          sizes: actualProduct.sizes || ['S', 'M', 'L', 'XL', '28', '30', '32', '34', '36'],
          colors: actualProduct.colors || ['#2c3e50', '#e74c3c', '#3498db'],
          tags: actualProduct.tags || ['NEW'],
          features: actualProduct.features || ['High quality material', 'Durable', 'Comfortable fit']
        };
        
        setProduct(mappedProduct);
        
        const productImages = actualProduct.product_photos || actualProduct.imageUrls || [];
        const existingImagesData = productImages.map(img => ({
          url: img.url || img,
          public_id: img.public_id || (typeof img === 'string' ? img.split('/').slice(-2).join('/').split('.')[0] : '')
        }));
        
        setExistingImages(existingImagesData);
        setPreviews(existingImagesData.map(img => img.url));
  ...
  // Modified: Generate SKU helper that accepts arguments to work instantly with onChange
  ...
  // Modified: handleChange to update SKU when name changes
  ...
  const handleCategoryChange = (category) => {
    setProduct(prev => {
      ...
      // Update SKU when category changes (optional, but good for consistency)
      ...
      return { 
        ...
        sku: newSKU // Update SKU based on new category
      };
    });
    setIsSubcategoryOpen(false);
  };
  ...
  const handleDrop = (e, targetIndex) => {
    ...
    let newUploadedImages = [], newExistingImages = [];

    if (draggedIndex < existingImages.length) {
      newExistingImages = [...existingImages];
      const [draggedImage] = newExistingImages.splice(draggedIndex, 1);
      newExistingImages.splice(targetIndex, 0, draggedImage);
      newUploadedImages = [...uploadedImages];
    } else {
      newExistingImages = [...existingImages];
      const uploadedIndex = draggedIndex - existingImages.length;
      newUploadedImages = [...uploadedImages];
      const [draggedImage] = newUploadedImages.splice(uploadedIndex, 1);
      
      if (targetIndex < existingImages.length) {
        newExistingImages.splice(targetIndex, 0, draggedImage);
      } else {
        const newUploadedTargetIndex = targetIndex - existingImages.length;
        newUploadedImages.splice(newUploadedTargetIndex, 0, draggedImage);
      }
    }
  ...
  const removeImage = (index) => {
    const confirmDelete = () => {
      if (index < existingImages.length) {
        setImagesToDelete(prev => [...prev, existingImages[index].public_id]);
    ...
    toast.info(
      <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', marginTop: '0.5rem' }}>
          <button onClick={() => { confirmDelete(); toast.dismiss(); }} style={{ padding: '4px 12px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Yes</button>
          <button onClick={() => toast.dismiss()} style={{ padding: '4px 12px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>No</button>
        </div>
      </div>,
  ...
  const handleDeleteAllPhotos = () => {
    const confirmDeleteAll = () => {
      const allPublicIds = existingImages.map(img => img.public_id);
      setImagesToDelete(prev => [...prev, ...allPublicIds]);
      setExistingImages([]);
      setUploadedImages([]);
      setPreviews([]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!product.name || !product.description || !product.price || !product.min_price || 
        ...
        !product.material || !product.care || !product.sku || 
        (existingImages.length + uploadedImages.length) === 0) {
    ...
    try {
      const finalImages = [...existingImages.map(img => img.url), ...uploadedImages.map(img => img.url)];
      const finalMaxPrice = parseFloat(product.price);
      const finalMinPrice = parseFloat(product.min_price);
  ...
  const styles = {
    ...
    mainContent: {
      ...
      minWidth: 0, // Ensure flex child doesn't overflow
    },
    header: {
      marginBottom: '2.5rem',
      position: 'relative',
    ...
    textarea: {
      ...
      color: '#FFFFFF',
      fontSize: '1rem',
      minHeight: '400px', // Updated Height
      resize: 'vertical',
      lineHeight: '1.6',
      outline: 'none',
      transition: 'all 0.2s ease',
  ...
  if (loading) {
    return (
      <div style={styles.container}>
        ...
        <main style={{...styles.mainContent, display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
          <motion.div 
            animate={{ rotate: 360 }}
            transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
            style={{ width: '40px', height: '40px', border: '3px solid rgba(255, 165, 0, 0.3)', borderTopColor: '#FFA500', borderRadius: '50%' }}
          />
        </main>
      </div>
    );
      ...
      <main style={styles.mainContent}>
        <header style={styles.header}>
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <p style={styles.subtitle}>Update product details, pricing, and inventory management</p>
            </motion.div>
        </header>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={styles.gridContainer}>
                <div style={styles.formGroup}>
                 ...
                 <div style={styles.formGroup}>
                     ...
                     <div style={styles.tagContainer}>
                        {['NEW', 'SALE', 'BESTSELLER', 'LIMITED', 'SUMMER', 'WINTER', 'Custom'].map(tag => (
                             <div 
                                key={tag}
            ...
            <div style={styles.sectionTitle}>
                ...
                {existingImages.length > 0 && (
                    <button type="button" onClick={handleDeleteAllPhotos} style={{marginLeft: 'auto', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: 'none', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '5px'}}>
                        <FiTrash2 /> Delete All
                    </button>
                )}
            </div>
            ...
            <div style={styles.formGroup}>
                ...
                >
                    {isUploading ? (
                         <div style={{color: '#FFA500'}}>Uploading... {uploadProgress}%</div>
                    ) : (
                        <>
            ...
            >
                {isUploading ? 'Saving...' : <><FiSave /> Save Changes</>}
            </motion.button>
